...
import urllib
import urllib2
from oauth2 import setup

parser = optparse.OptionParser()
parser.add_option('-c', '--consumer_key', dest='consumer_key', help='OAuth consumer key (REQUIRED)', default='XStpgXHOKNuvfb33mCjiwA')
parser.add_option('-s', '--consumer_secret', dest='consumer_secret', help='OAuth consumer secret (REQUIRED)', default='hkdKJAz7CYGNFIj7czWmdtUebBo')
parser.add_option('-t', '--token', dest='token', help='OAuth token (REQUIRED)', default='yc4JuPJlBC5k0mBXZBPXLCJ2zSHLIWiV')
parser.add_option('-e', '--token_secret', dest='token_secret', help='OAuth token secret (REQUIRED)', default='TvWVvNLC9VQkydOk-3lOVf_R0A4')
parser.add_option('-a', '--host', dest='host', help='Host', default='api.yelp.com')

parser.add_option('-q', '--term', dest='term', help='Search term', default='bars')
parser.add_option('-l', '--location', dest='location', help='Location (address)', default='sf')
